...
@Component({
  selector: 'app-rent-bulkapprove',
  templateUrl: './rent-bulkapprove.component.html',
  styleUrls: ['./rent-bulkapprove.component.scss']
})
export class RentBulkapproveComponent implements OnInit {
  ...
  allAdminSelection(event:any){
    if(event.target.checked){
      this.data.map((item:any)=>{
        if(item.adminStatus == 'Pending' && item.financeStatus == 'Pending' && this.authS.hasApprovePermission('rent',(item.isTds)?item.totalRentAfterDeduction:item.totalMonthlyRent, item.verifyStatus)){
          item.checked = true;
        }   
      });
    }else{
  ...
  allFinanceSelection(event:any){
    if(event.target.checked){
      this.data.map((item:any)=>{
        if(item.adminStatus == 'Approved' && item.financeStatus == 'Pending' && this.authS.hasFApprovePermission('rent',(item.isTds)?item.totalRentAfterDeduction:item.totalMonthlyRent, item.fverifyStatus)){
          item.checked = true;
        }   
      });
    }else{
  ...
  adminApproveAll(){
    ...
    for(let i = 0;i<this.checkedData.length;i++){
      ...
      this.api.updateRentBill(data,this.checkedData[i].id).subscribe(data=>{
