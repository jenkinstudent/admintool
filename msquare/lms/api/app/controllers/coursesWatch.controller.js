...
exports.get = async (req,res)=>{
  const condition = req.body.condition || {};
    ...
    const data = await CoursesWatch.find(query).skip((page - 1) * limit).limit(limit).sort({createdAt:1})
    .populate("courseId").populate("employeeId").exec();
    const totalCount = await CoursesWatch.countDocuments(query);
    const totalData = await CoursesWatch.find(query).populate("courseId").populate("employeeId");
