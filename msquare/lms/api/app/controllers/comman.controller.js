                                    ...
                                    let depInterval = setInterval(() => {
                                        if (department != "") {
                                            ...
                                            if(employee.DateofBirth != "" && employee.DateofBirth != null && employee.DateofBirth != undefined){
                                                ...
                                                var day = dateParts[0];
                                                dateObj = moment({ year, month: getMonthNumber(month), day }).tz('Asia/Kolkata');
...
exports.deleteModuleAllocation = async (req, res) => {
    let modWatch = await ModulesWatch.find({
        moduleId: req.params.id
    })
    let data = [];
    let count = 0;
    await Promise.all(modWatch.map(async modData => {
        let empModWatch = await ModulesWatch.find({employeeId: modData.employeeId,index: {$gt: modData.index}}).sort({index:1});
        if(empModWatch.length > 0){
            if(!empModWatch[0].unlock && modData.unlock){
                await ModulesWatch.findByIdAndUpdate(empModWatch[0]._id,{$set:{unlock:true}});
            }
            await QuizScore.findByIdAndRemove(modData.quizReference);
        }else{
            await QuizScore.findByIdAndRemove(modData.quizReference);
        }
        count++;
    ...
    if(count == modWatch.length){
        await ModulesWatch.deleteMany({moduleId:req.params.id});
    }
    // await ModulesWatch.updateMany({moduleId:'648c439c99f3f686b47c826f',index:3,unlock:false},{$set:{unlock:true}});
    res.status(200).send({
        status:"success",
        message : "All Modules deleted",
        data: data
    });
}

exports.deleteProgramAllocation = async (req, res) => {
    let prgWatch = await ProgramsWatch.find({
        programId: req.params.id
    });
    let data = [];
    let count = 0;
    await Promise.all(prgWatch.map(async prgData => {
        let empPrgWatch = await ProgramsWatch.find({employeeId: prgData.employeeId,index: {$gt: prgData.index}}).sort({index:1});
        if(empPrgWatch.length > 0){
            if(!empPrgWatch[0].unlock && prgData.unlock){
                await ProgramsWatch.findByIdAndUpdate(empPrgWatch[0]._id,{$set:{unlock:true}});
            }
        }
        count++;
    }));

    if(count == prgWatch.length){
        await ProgramsWatch.deleteMany({programId:req.params.id});
    }
    res.status(200).send({
        status:"success",
        message : "All Programs deleted",
        data: data
    });
}

exports.deleteCourseAllocation = async (req, res) => {
    let courWatch = await CoursesWatch.find({
        courseId: req.params.id
    });
    let data = [];
    let count = 0;
    await Promise.all(courWatch.map(async courData => {
        let empCourWatch = await CoursesWatch.find({employeeId: courData.employeeId,index: {$gt: courData.index}}).sort({index:1});
        if(empCourWatch.length > 0){
            if(!empCourWatch[0].unlock && courData.unlock){
                await CoursesWatch.findByIdAndUpdate(empCourWatch[0]._id,{$set:{unlock:true}});
            }
        }
        count++;
    }));

    if(count == courWatch.length){
        await CoursesWatch.deleteMany({courseId:req.params.id});
    }
    res.status(200).send({
        status:"success",
        message : "All Courses Delted",
        data: data
    });
}

exports.deleteLearningActivityAllocation = async (req, res) => {
    await LearningActivityWatch.deleteMany({programId: req.params.id});
    await LearningActivityModule.deleteMany({programId: req.params.id});
    res.status(200).send({
        status:"success",
        message : "All Learning Activity deleted",
    });
}