...
exports.getAllQuizScore = async (req,res)=>{
    ...
    const data = await QuizScore.find(query).skip((page - 1) * limit).limit(limit).sort({createdAt:1})
    .populate("quizId").populate("employeeId").exec();
    const totalCount = await QuizScore.countDocuments(query);
    const totalData = await QuizScore.find(query).populate("quizId").populate("employeeId");
