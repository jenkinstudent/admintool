...
exports.get = async (req, res) => {
  const condition = req.body.condition || {};
    ...
    const data = await ModulesWatch.find(query).skip((page - 1) * limit).limit(limit).sort({createdAt:1})
    .populate("moduleId").populate("employeeId").exec();
    const totalCount = await ModulesWatch.countDocuments(query);
    const totalData = await ModulesWatch.find(query).populate("moduleId").populate("employeeId");
...
exports.getByStateAndDept = async (req, res) => {
  ...
  const condition = req.body.condition || {};
    ...
    const data = await ModulesWatch.aggregate(
      [
        {
          $match:query
        },
        ...
        { $unwind: '$empData' },
        { $unwind: '$modData' },
        { $group : { _id: { state: '$empData.stateEmp', department: '$empData.department',moduleId: '$modData._id' },
                    count: { $sum: 1 },
                    moduleName: { $addToSet: "$modData.title" } 
                  }
        },
        { $skip : (page - 1) * limit },
        { $limit: limit },
    ...
    // const data = await ModulesWatch.find(query).skip((page - 1) * limit).limit(limit).sort({createdAt:1})
    // .populate("moduleId").populate("questionbank").populate("employeeId").exec();
    const totalData = await ModulesWatch.aggregate(
      [
        {
          $match:query
        },
