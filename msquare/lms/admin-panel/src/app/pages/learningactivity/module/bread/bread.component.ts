...
export class BreadComponent implements OnInit{
  ...
  getQuestionBank(){
    this.qbData = [];
    this.apiS.getAllActiveQuestionBankLearningActivity().subscribe(data=>{
  ...
  save(){
    ...
    if(this.documentspdf.length == 0 && this.onePagers.length == 0 && this.youtubes.length == 0){
      this.toastr.error("Please upload atleast one media");
      this.loader=false;
      return;
    } 
  ...
  clearFilter(){
    ...
    this.getIncrementalCode();
    this.getQuestionBank();
    this.router.navigate(['/pages/learning-activity/module']);
  }
