...
export class CourseComponent implements OnInit{
  ...
  constructor(private title: Title,public http: HttpClient,public api:ApiService,public toast:ToastrService,public router:Router,public user: AuthenticationService,
    ...
    this.title.setTitle("Course - Fusion Microfinance");
  }
  ...
  getData(){
    this.loading = true;
    this.api.allCourseLeaningActivity().subscribe(data=>{
      this.data = data.data;
    });
  ...
  editCourse(id:String){
    this.router.navigate(['/pages/learning-activity/course/activity/edit'],{
      queryParams:{id:id}
    });
  }

  deleteCourse(id:any){
    this.api.deleteCourses(id).subscribe(data=>{
      if(data.status){
  ...
  confirm(id:any) {
    Swal.fire({
      ...
      text: 'Deleting your course will remove all of your information from database.',
      icon: 'warning',
  ...
  changeCourseStatus(item:any,event:any){
    let status = "1";
    if(!event.target.checked){
      status = "0";
    }

    this.api.updateStatusCourse(item._id,status).subscribe(data=>{
      if(status == "0"){
        item.status = "0";
        this.toast.error("Course Status Updated");
      }else if(status == "1"){
        item.status = "1";
