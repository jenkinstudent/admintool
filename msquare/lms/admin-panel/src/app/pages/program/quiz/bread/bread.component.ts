...
export class BreadComponent implements OnInit {
  ...
  save(needEdit: boolean) {
    ...
    if (this.edit) {
      this.apiS.updateQuestioBank(this.quizTitle, this.keywords, this.description, this.questionsCount, this.quizTime, this.fusionLearningBank, false, this.expiryDate, this.learningLiveDate, this.learningLiveTime, this.thumbnailLink, 1, this.qbId).subscribe(result => {
        if (result.status) {
          this.toastr.success(result.message);
          this.clearFilter();
        } else {
      ...
      this.apiS.createQuestioBank(this.questionBankCode, this.quizTitle, this.keywords, this.description, this.questionsCount, this.quizTime, this.fusionLearningBank, false, this.expiryDate, this.learningLiveDate, this.learningLiveTime, this.thumbnailLink, 0).subscribe(result => {
        if (result.status) {
          this.toastr.success(result.message);
          this.clearFilter();
          if (needEdit) {
            this.router.navigate(["/pages/programs/quiz/activity/edit"], { queryParams: { id: result.data._id } });
          }
        } else {
          this.toastr.error(result.message);
        }
  ...
  clearFilter() {
    ...
    this.router.navigate(['/pages/programs/quiz']);
  }
